...
class GraphRotatedLabel(MDLabel):
...
class Graph(Widget):
    ...
    '''Color of the border, defaults to black
    '''

    label_options = DictProperty()
    '''MDLabel options that will be passed to `:class:`kivy.uix.MDLabel`.
    '''
    ...
    def _redraw_y(self, *args):
        ...
        grids.extend([None] * (n_labels - len(grids)))
        for k in range(grid_len, n_labels):
            grids[k] = MDLabel(font_size=font_size, **self.label_options)
            self.add_widget(grids[k])
