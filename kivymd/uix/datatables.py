...
class CellRow(
    ...
    def apply_selection(self, table_data, index, is_selected):
        ...
        if table_data._rows_number in table_data.current_selection_check:
            for index in table_data.current_selection_check[table_data._rows_number]:
                if (
                    self.index
                    in table_data.current_selection_check[table_data._rows_number]
                ):
        ...
        if self.sort_action:
            ...
            if sorted_data:
                self.table_data.row_data = (
                    sorted_data if inst.icon == "arrow-down" else sorted_data[::-1]
                )
    ...
    def set_text_from_of(self, direction):
        ...
        if self.pagination:
            ...
            self.pagination.ids.label_rows_per_page.text = (
                f"{self._current_value}-{self._to_value} of {len(self.row_data)}"
            )
...
class MDDataTable(ThemableBehavior, AnchorLayout):
    ...
    def create_pagination_menu(self, interval):
        menu_items = [
            {"text": f"{i}"}
            for i in range(self.rows_num, len(self.row_data) // 2, self.rows_num)
        ]
