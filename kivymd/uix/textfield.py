...
__all__ = ("MDTextField", "MDTextFieldRect", "MDTextFieldRound","MDCardTextField")
...
class MDTextFieldRect(ThemableBehavior, TextInput):
    ...
    def anim_rect(self, points, alpha):
        ...
        Animation(a=alpha, d=(d_color if self.line_anim else 0)).start(instance_color)
...
class MDTextField(ThemableBehavior, TextInput):
    ...
    def on_focus(self, *args):
        ...
        if self.focus:
            ...
            _fill_color = self.fill_color
            _fill_color[3] = self.fill_color[3] + 0.1
            Animation(fill_color=_fill_color, duration=0.2, t="out_quad").start(self)
            if not self.text:
            ...
            if self._get_has_error():
                ...
                Animation(duration=0.2, color=(1, 1, 1, 1)).start(self._hint_lbl)
    ...
    def _get_has_error(self):
        if self.error or all(
            [self.max_text_length is not None and len(self.text) > self.max_text_length]
        ):
            has_error = True
        else:
    ...
    def _get_max_text_length(self):
        ...
        return sys.maxsize if self.max_text_length is None else self.max_text_length
    ...
    def _set_message_mode(self, instance, text):
        ...
        if self.helper_text_mode == "persistent":
            self._anim_current_error_color(self.theme_cls.disabled_hint_text_color, 0.1)
...
class MDCardTextField(MDBoxLayout, RectangularElevationBehavior, ThemableBehavior):
    """
    ...
    def __init__(self, **kwargs):
        ...
        if self.icon_left:
            self.icon_left_widget = MDIconButton(
                ...
                font_name=self.icon_left_font_name, font_size=self.icon_left_font_size, pos_hint={'center_y':0.5})
        ...
        if self.icon_right:
            self.icon_right_widget = MDIconButton(
                ...
                font_name=self.icon_right_font_name, font_size=self.icon_right_font_size, pos_hint={'center_y':0.5})
            self.add_widget(self.icon_right_widget)
            self.multiline = False

    def on_icon_left(self, instance, value):
        self.icon_left_widget.icon = value
        self.icon_left_widget.pos_hint={'center_y':0.5}
        try:
            self.add_widget(self.icon_left_widget, index=1)
    ...
    def on_icon_right(self, instance, value):
        ...
        self.icon_right_widget.pos_hint={'center_y':0.5}
        try:
            self.add_widget(self.icon_right_widget)
        except AttributeError:
            self.children.clear()
