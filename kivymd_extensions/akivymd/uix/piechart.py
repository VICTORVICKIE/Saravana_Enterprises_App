...
class AKPieChart(ThemableBehavior, BoxLayout):
    ...
    color_mode = OptionProperty(
        "colors", options=["primary_color", "accent_color","Custom"]
    )  # not solved
        ...
        for title, value in items.items():
            with self.canvas.before:
                ...
                if self.color_mode == "colors":
                    ...
                    custom_color = self.custom_colors[0][list(items)[color_item]]

                    color = get_color_from_hex(custom_color)
