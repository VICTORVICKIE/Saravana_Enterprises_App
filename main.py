...
class BillingApp(MDApp):
	...
	def product_carousel(self, *args):
		for product_image_name in listdir(PRODUCTS_DIR):
			...
			self.kv.ids.product_carousel.add_widget(
				CustomCarouselItem(
					source=path_to_product_image, name_product=self.product_pic_dict[product_image_name],
				)
			)

	# Carousel AutoPlay Scheduler
	def product_carousel_autoplay(self):
		self.product_carousel_event = Clock.schedule_interval(self.product_carousel_callback, 5)
	...
	def search_menu(self,text="", search=False):
		...
		if text:
			for search_term in search_list:
				if search:
					condition = text in search_term or text in search_term.lower() or text in search_term.upper()
					if condition:
						search_menu_items.append({"text": f"{search_term}"})
		
		self.search_drop_menu = MDDropdownMenu(
			caller=self.kv.ids.cus.ids.search_bar,
	...
	def area_menu(self):
		...
		self.area_menu = MDDropdownMenu(
			...
			width_mult = 4,
			position = 'bottom',
			selected_color = self.theme_cls.accent_light_hue
		)
	...
	def add_shop_dialog(self):
		if not self.shop_dialog:
			self.shop_dialog = MDDialog(
				...
				buttons = [
					MDFlatButton(
						text = "CANCEL", text_color = self.theme_cls.primary_color, on_release = self.close_shop_dialog
					),
					MDFlatButton(
						text = "ADD", text_color = self.theme_cls.primary_color, on_release = self.get_shop_details
					),
				],
